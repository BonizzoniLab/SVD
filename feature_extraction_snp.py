...
def get_info_freebayes(chrom,pos,ref,alt,info,format,sample,freebayes):
	...
	if sample is not 'null' and sample[format.index('DP')] is not '0':
		...
		try:
			freebayes.AF=float(freebayes.AO)/float(freebayes.DP)
		except:
			freebayes.AF='.'
	else:
		freebayes.DP=0
	...
	try:
		freebayes.QB=float(sample[format.index('QA')])/float(freebayes.AO)
	except:
...
def get_info_vardict(chrom,pos,ref,alt,info,format,sample,vardict):
	...
	if sample is not 'null' and sample[format.index('DP')] is not '0' :
		...
		try:
			vardict.AF=float(vardict.AO)/float((vardict.DP))
		except:
			vardict.AF=float(0)
	else:
		vardict.AO='.'
...
def get_info_platypus(chrom,pos,ref,alt,info,format,sample,platypus):
	...
	if sample is not 'null' and int(info[14].split('=')[1]) is not '0' :
		...
		try:
			platypus.AF=float(platypus.AO)/float((platypus.DP))
		except:
			platypus.AF=float(0)
	else:
		platypus.AO='.'
...
def get_info_gatk(chrom,pos,ref,alt,info,format,sample,gatk):
	...
	if sample is not 'null' and sample[format.index('DP')] is not '0' :
		...
		try:
			gatk.AF=float(gatk.AO)/float((gatk.DP))
		except:
			gatk.AF=float(0)
	else:
		gatk.AO='.'
	...
	try:		
		gatk.MQ0F=float(gatk.MQ0)/float(gatk.DP)
	except:
		gatk.MQ0F=float(0)
...
def switch_snp(dictionary,ID,index,chrom,pos,ref,alt,info,format,sample):
...
def read(iterable,index,dictionary):
	...
	for line in iterable:
		...
		if line.startswith('#'):
			...
			if len(alt)>1 or len(ref)>1 or SAMPLE.startswith('0/0:'):
				continue
			else:
				if "VD" in format:
					if sample[format.index('VD')] is '0':
						...
						switch_snp(dictionary,ID,index,chrom,pos,ref,alt,info,format,sample)
				else:	
					switch_snp(dictionary,ID,index,chrom,pos,ref,alt,info,format,sample)
